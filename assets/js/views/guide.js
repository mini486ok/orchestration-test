...
# 브라우저에서 http://localhost:8000 접속</code></pre>
  ...
  },
  {
    ...
    html: `
      ...
      <p><b>① Vector DB</b> — 도구별 설명 텍스트를 임베딩 모델(기본 <code>bge-m3</code>)로 벡터화한 인덱스.</p>
      <ul>
        <li>검색 방식: <code>vector</code>(임베딩 코사인) / <code>keyword</code>(BM25, 임베딩 불필요) / <code>hybrid</code>(가중합 α).
      ...
      <b>평가·비교</b> 화면에서 전체 카탈로그 전략과 vector/graph DB 전략을 나란히 돌려 정확도(F1)와 효율을 직접 비교해 보세요.</p>`,
  },
  {
    ...
    html: `
      ...
      <p class="hint">정식 정량 비교(P/R/F1 등)는 <b>평가·비교</b> 화면을, 빠른 즉석 확인·시연은 이 <b>실시간 테스트</b>를 사용하세요.</p>`,
  },
  {
    title: '📏 평가 지표 설명',
    html: `
